...
filter_cities = ["Las Vegas"]
filter_categories = ["Hotels"]
...
with open(r"dataset\business.json", 'r', encoding='utf-8') as f:
    for line in f:
        ...
        business_id2business[business_id] = j
        name = j["name"]
...
with open(r"dataset\review.json", 'r', encoding='utf-8') as f:
    for line in f:
        ...
        city = business_id2city[business_id]
        if city in filter_cities:
...
for category in filter_categories:
    with open("data/Las_Vegas-"+category+".p","rb") as f:
        previous_reviews, previous_selected_category_business_id2business, previous_selected_category_user_id2user = pickle.load(f)

    reviews = category2reviews[category]

    selected_category_business_id2business = dict()
    selected_category_user_id2user = dict()
    for i,review in enumerate(reviews):
        previous_review = previous_reviews[i]
        if review["review_id"] == previous_review["review_id"]:
            review["words"] = previous_review["words"]
        else:
            review["words"] = process_words(review["text"], "en", stem=True)
        selected_category_business_id2business[review["business_id"]] = business_id2business[review["business_id"]]
        selected_category_user_id2user[review["user_id"]] = user_id2user[review["user_id"]]
    print(i)
    with open("data/Las_Vegas-"+category+".p","wb+") as f:
        pickle.dump([reviews, selected_category_business_id2business, selected_category_user_id2user],f)
