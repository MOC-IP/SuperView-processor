...
def get_radical_ngrams(business_id ,min_instances = 5, max_number_of_results_to_print = 100, filter_results = True):
    try:
    ...
    return get_radical_ngrams_from_reviews(reviews ,min_instances = min_instances, max_number_of_results_to_print = max_number_of_results_to_print, filter_results = filter_results)

def get_radical_pairs(business_id ,min_instances = 5, max_number_of_results_to_print = 100, filter_results = True):
    try:
        reviews, selected_business_name = get_business_data(business_id)
    except Exception:
        reviews, selected_business_name = get_business_data_local( business_id)
    return get_radical_adj_noun_pairs_from_reviews(reviews ,min_instances = min_instances, max_number_of_results_to_print = max_number_of_results_to_print, filter_results = filter_results)

def get_radical_ngrams_from_reviews(reviews, min_instances = 5, max_number_of_results_to_print = 100, filter_results = True):
    ngram2rating = dict()
    ...
    if filter_results:
        filter_for_completeness(word_score_instances)
        filter_words_by_sentiment(word_score_instances)
        wordlist = ["room","food","service","location","room","clean","good","place","bad","dessert"]
        filter_words_by_word2vec_distance(word_score_instances,wordlist)
...
def get_radical_adj_noun_pairs_from_reviews(reviews, min_instances = 5, max_number_of_results_to_print = 100, filter_results = True):
    pair2rating = dict()
    for review in reviews:
        star = float(review["stars"])
        pairs = detectNLP.get_adj_noun_pairs(review["text"])
        for pair in pairs:
            pair_str = " ".join(pair)
            if pair_str not in pair2rating:
                pair2rating[pair_str] = [0.0, 0]
            pair2rating[pair_str][0] += star
            pair2rating[pair_str][1] += 1

    word_score_instances = []
    for pair_str in pair2rating:
        pair2rating[pair_str][0] = pair2rating[pair_str][0] / pair2rating[pair_str][1]
        if pair2rating[pair_str][1] > min_instances:
            word_score_instances.append([pair_str] + pair2rating[pair_str])
