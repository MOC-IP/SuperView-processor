...
def min_similar(word, min_similarity= 0.5):
    queries = [w for w in word.vocab if w.is_lower == word.is_lower and w.prob >= -15]
    querry_sim = map(lambda w: (w, word.similarity(w)), queries)
    querry_sim = filter(lambda w: w[1] > min_similarity, querry_sim)
    querry_sim = sorted(querry_sim, key=lambda w: w[1], reverse=True)
    by_similarity = list(map(lambda w: w[0], querry_sim))
