...
def compare_sentiment_analysis(reviews, business_id2business, output_file = "data/compare_sentiment_analysis.txt"):
    ...
    vader2error = dict()
    corenlp2error = dict()
    rating2textblobError = dict()
    rating2vaderError = dict()
    rating2corenlpError = dict()
    ...
    for review in reviews:
        ...
        score = float(review["normalized_bias"])
        if review["stars"] not in rating2textblobError:
            rating2textblobError [review["stars"]]= [0.0, 0]
            rating2vaderError[review["stars"]]    = [0.0, 0]
            rating2corenlpError[review["stars"]]  = [0.0, 0]
        rating2textblobError[review["stars"]][1] += 1
        rating2vaderError[review["stars"]][1] += 1
        rating2corenlpError[review["stars"]][1] += 1
        ...
        textblob_error = abs(score - textblob_score)
        textblob2error[review["business_id"]][0] += textblob_error
        rating2textblobError[review["stars"]][0] += textblob_error

        vader_score = get_sentiment_vader(text)
        if abs(score - vader_score) <= 0.125:
            vader2acc[review["business_id"]][0] += 1

        vader_error = abs(score - vader_score)
        vader2error[review["business_id"]][0] += vader_error
        rating2vaderError[review["stars"]][0] += vader_error

        core_nlp_score = get_average_sentiment_coreNLP(text)
        if abs(score - core_nlp_score) <= 0.125:
            corenlp2acc[review["business_id"]][0] += 1

        corenlpError = abs(score - core_nlp_score)
        corenlp2error[review["business_id"]][0] += corenlpError
        rating2corenlpError[review["stars"]][0] += corenlpError
    ...
    with open(output_file, "a") as f:
        ...
        print_str = "Rating\tTextblob Error\tVader Error\tCoreNLP Error"
        print(print_str)
        f.write(print_str + "\n")
        for rating in range(1,6):
            print_str = str(rating)
            err = rating2textblobError[rating]
            print_str += "\t" + str(round(err[0] / err[1], 4))
            err = rating2vaderError[rating]
            print_str += "\t" + str(round(err[0] / err[1], 4))
            err = rating2corenlpError[rating]
            print_str += "\t" + str(round(err[0] / err[1], 4))
            print(print_str)
            f.write(print_str + "\n")
